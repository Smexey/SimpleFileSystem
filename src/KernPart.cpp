...
void KernPart::close(KernFile* kernfile) {
    ...
    if (it != mapOpen.end()) {
        if (it->second->readers < 2 && it->second->writers == 0 && it->second->waitWriters == 0) {
            ...
            if (mapOpen.empty() && marked) signal(deleteSem);
            return;
        } else if (it->second->writers < 2 && it->second->readers == 0 &&
                   it->second->waitWriters == 0 && it->second->waitReaders == 0) {
            delete (it->second);
            mapOpen.erase(it);
...
char KernPart::format() {
    ...
    bitVect->format();
    for (int i = 0; i < ClusterSize; i++) rootCache[i] = 0;
    marked = false;
    return 1;
}
    ...
    for (ClusterNo i = 0; i < ClusterRootNo; i++)
        if (rootB[i] != 0) {
            ...
            if (i < ClusterRootNo / 2) {
                // lvl1ind
                clEnt = (Entry*)rootBuf1;
                for (ClusterNo j = 0; j < ClusterEntryNo; j++) return ((clEnt + j)->nameEq(fname));
...
void KernPart::writeEntryToBuffer(char* fname, char* buffer, ClusterNo pos, ClusterNo off) {
    ...
    for (unsigned i = 0; i < fileNameLen; i++) {
        ...
        if (!flag) entry[off].name[i] = fname[i];
        if (flag) entry[off].name[i] = ' ';
    }
    ...
    entry[off].size = 0;
    // cout << "wrote " << entry[off].name << " ext " << entry[off].ext << " in entry " << off <<
    // "INDEXCLUSTER:" << entry[off].indexCluster << endl;
...
ClusterNo KernPart::newClusterFS(char* bufferRoot, char* buffer, ClusterNo pos, ClusterNo off) {
    ...
    for (ClusterNo i = 0; i < ClusterSize; i++) buffer[i] = 0;
    return k;
}
...
void KernPart::freeCluster(ClusterNo clno) { bitVect->free(clno); }
